  ...
  return (
    <div className="px-4 pt-6 pb-28 page-enter">
      <div className="flex items-center gap-2.5 mb-6">
        ...
        <h1 className="text-lg font-sans font-bold">Leaderboard</h1>
      </div>

      {loading ? (
        <p className="text-center text-xs text-[var(--color-text-muted)] font-sans animate-pulse mt-12">Loading...</p>
      ) : entries.length === 0 ? (
        <div className="text-center mt-14">
          ...
          <p className="text-sm font-sans font-semibold text-[var(--color-text)] mb-1">No explorers yet</p>
          <p className="font-sans text-xs text-[var(--color-text-muted)]">Be the first to earn XP!</p>
        </div>
      ) : (
        <>
          {top3.length > 0 && (
            ...
            <div className="space-y-2">
              {rest.map((entry, i) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[11px] font-sans font-bold w-8 text-center text-[var(--color-text-muted)] tabular-nums">
                      {entry.rank}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-sans font-semibold text-sm truncate">
                        {entry.display_name}
                        {isMe && (
                          <span className="text-[8px] text-white bg-[var(--color-primary)] px-1.5 py-0.5 rounded-full ml-1.5 font-medium uppercase">
                            you
                          </span>
                        )}
                      </p>
                      <p className="text-[9px] text-[var(--color-text-muted)] font-sans">Level {entry.level}</p>
                    </div>
                    <div className="flex items-center gap-1 text-[var(--color-primary)]">
                      <Zap size={13} />
                      <span className="text-[11px] font-sans font-bold tabular-nums">{entry.total_xp}</span>
                    </div>
                  </div>
                );
              })}
